...
tripSchema.pre("validate", async function (next) {
  ...
  if (vehicle.status !== "Available") {
    return next(
      new Error(`Vehicle is not available (status: ${vehicle.status})`)
    );
  }

  if (driver.status !== "Available") {
    return next(
      new Error(`Driver is not available (status: ${driver.status})`)
    );
  }
...
/**
 * Dispatch Trip
 * Automatically marks both Vehicle and Driver as On Trip.
 */
...
/**
 * Complete Trip
 * Automatically restores Vehicle and Driver to Available.
 * Updates vehicle odometer and trip fuel consumption.
 */
